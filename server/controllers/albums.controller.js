...
const getAlbumNombre = async (req, res) => {
	try {
		const { titulo } = req.body;
		console.log(req.body);
		const [result] = await pool.query(`SELECT * FROM albumes WHERE titulo= ?`, [req.body.titulo]);

		if (result.length === 0)
			return res.status(404).json({ msg: `El álbum con nombre ${req.body.titulo} NO EXISTE.` });
...
const createAlbum = async (req, res) => {
	try {
		const { titulo, fechaLanzamiento, discografia } = req.body;
		...
		const [result] = await pool.query("INSERT INTO albumes VALUES (default,?,null,?,?)", [
			titulo,
			fechaLanzamiento,
			discografia,
		]);
...
const updateAlbum = async (req, res) => {
	try {
		console.log("UPDATE");
		console.log(req.body);
		const { titulo, fotoAlbum, fechaLanzamiento, discografia } = req.body;
		const [result] = await pool.query(
			"UPDATE albumes SET titulo = IFNULL(?,titulo), fotoAlbum = IFNULL(?,fotoAlbum), fechaLanzamiento = IFNULL(?,fechaLanzamiento),discografia = IFNULL(?,discografia) WHERE idAlbum = ?",
			[titulo, fotoAlbum, fechaLanzamiento, discografia, req.params.id]
		);
...
const getIdAlbumAndFilter = async (req, res) => {
	try {
		...
		const [result] = await pool.query(
			"SELECT axc.isrc, a.nombreArtistico AS artista, a.fotoArtista, c.titulo AS tituloCancion, c.fechaLanzamiento, c.duracion, g.nombreGenero, al.titulo AS tituloAlbum,al.fotoAlbum,c.rutaCancion FROM artistaXCanciones AS axc INNER JOIN artistas AS a ON axc.idArtista = a.idArtista INNER JOIN canciones AS c ON axc.isrc = c.isrc INNER JOIN generos AS g ON g.idGenero = c.idGenero INNER JOIN albumes as al  ON al.idAlbum = c.idAlbum WHERE al.idAlbum = ?;",
			[idAlbumBuscado]
		);
...
export { getAlbum, getAlbumes, getAlbumNombre, createAlbum, updateAlbum, eliminarAlbum, getIdAlbumAndFilter };
